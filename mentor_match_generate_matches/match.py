...
def handler(event, context):
    ...
    mentors = json.loads(s3_connection.get_object(
        Bucket = bucket,
        Key = mentors_key
    )["Body"].read().decode("utf-8"))
    mentees = json.loads(s3_connection.get_object(
        Bucket = bucket,
        Key = mentees_key
    )["Body"].read().decode("utf-8"))
    ...
    print((matched_mentors, matched_mentees, unmatched_bonus))
    ddb_connection = boto3.client("dynamodb")
    response = ddb_connection.put_item(
        TableName=ddb_table, Item={"id": {"S": match_id}, "status": {"S": "COMPLETED"}}
    )
